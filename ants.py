...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.queen:
                return
...
class Insect(object):
    ...
    def reduce_armor(self, amount):
        ...
        #ALSo deals damage to an insect
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(self.min_range, self.max_range):
            while look_up != hive:
                if range_count >= self.min_range and range_count <= self.max_range and look_up.bees:
                    return random_or_none(look_up.bees)
                else:
                    look_up = look_up.entrance
                    range_count += 1
        return None
        # END Problem 3B
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        doomed_bee = random_or_none(self.place.bees)
        Insect.reduce_armor(doomed_bee, doomed_bee.armor)
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if self.queen:
            ...
            place_lst=[]
            while look_up != None:
                place_lst.append(look_up)
                look_up = look_up.exit

            ants = []
            for place in place_lst:
                if place.ant != None:
                    if place.ant.container == True and place.ant.ant:
                        ants.append(place.ant.ant)
                    ants.append(place.ant)

            for ant in ants:
                if not isinstance(ant, QueenAnt):
                    if ant.container == True and ant not in QueenAnt.buffed:
                        ant.damage = ant.damage * 2
                        QueenAnt.buffed.append(ant)
                        if ant.ant and ant.ant not in QueenAnt.buffed:
                            ant.ant.damage = ant.ant.damage * 2
                            QueenAnt.buffed.append(ant.ant)
                    elif ant not in QueenAnt.buffed:
                        ant.damage = ant.damage * 2
                        QueenAnt.buffed.append(ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.queen and self.armor == 0:
            bees_win()
...
def apply_effect(effect, bee, duration):
    ...
    def helper(colony):
        '''used so we can access colony'''
        nonlocal start_time
        start_time += 1
        if start_time <= duration:
            return effect(og_action)(colony)
        return og_action(colony)
    bee.action = helper
